##########################################################################
## Title: Create RFF look-up Tables.R
## Purpose: To create a look-up table of the RFF scenarios              ####
##      Look-up tables will contain the ratio of (1)mortality and (2) population
##          for each scenario to the mean (1) mortality and (2) population values
##      Ratios will be used as input to the Reduced Form Tool ##
## Written by: Industrial Economics, Inc. Edited by: EPA OAP
...
#### Set working directory and read in inputs ####
  #Working Directories
    ...
    input_rff   <- file.path('input','RFF','lookup_tables','input_data')
    input_ifp   <- file.path('input','IFP') 
    #input2  <- file.path('Code','InputData','RFF Lookup Tables')
    outpath <- file.path('input','RFF','lookup_tables')
    
  #input data (takes a few minutes to load, these files are large)
    mx  <- read.csv(paste0(input_rff,"/mx_trajectories.csv"))
    pop <- read.csv(paste0(input_rff,"/pop_trajectories.csv"))
    #subset for years <=2100
    #must crosswalk mortality year ranges to years first
    Cross.year <- read_xlsx(file.path(input_ifp,"Crosswalk RFF Mort Year.xlsx"))
  ...
  #IFs resp adjustments - requires crosswalking IFs country ID to RFF country ID
    IFsFactors   <- read.csv(paste0(input_ifp,"adjustment_factors.csv"))  
    IFsCountry   <- read.csv(paste0(input_ifp,"country_codes.csv"))
    IFsCtryCross <- read.csv(paste0(input_ifp,"country_crosswalk.csv"))
...
#### Calculate ratios for all-age population to mean  ####
    ...
    pop <- readRDS(file.path(outpath,"All Population Data_through 2100.rds"))
    
    #read in mean population
    mean.pop <- read.csv(file.path(outpath,"rff_pop_means.csv"))
...
#### Calculate ratios for all-age Mortality to mean  ####
    ...
    mx<-readRDS(file.path(outpath, "All Mortality Data_through 2100.rds"))
    ...
    mean.mort<-read.csv(file.path(outpath,"rff_mort_means.csv"))
