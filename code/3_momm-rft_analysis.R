#####
## Title:   3_momm-rft_analysis.R
## Purpose: This file will read all output files from the Methane-Ozone Mortality
##          Model-Reduced Form Tool and generate aggregate dataframes
##          for use in the manuscript graphics script.
##          
...
inputsPath   <- "." %>% file.path("output","rft")
outputsPath  <- "." %>% file.path("output", "summary_data")
#original scghg
...
c_iteration  <- inputsPath %>% list.files(pattern = "\\parquet") %>%
  (function(x){sub("\\_momm_rft.parquet", "", x)}) %>%
...
##1. Read in all physical and annual damages from all trials
# If df is too large...
...
load_raw = 1
reload <- ifelse(file.exists(outputsPath %>% file.path("annual_impacts_thru2050.parquet")) 
                 & load_raw ==0,1,0)

if (reload ==1){
  df_fraw_allnat <- read_parquet(outputsPath %>% file.path("annual_impacts_thru2050.parquet"))
  ...
  df_fraw_allnat <- 
    pblapply(1:length(c_iteration), function(i){
      ...
      data_i    <- data_i   %>% 
        ...
        filter(ModelYear <= 2050) %>%                             #filter for first part of time series (to minimize df size)
        select(-c(contains("physical"),"Region","SuperRegion","LocID")) 
  ...
  ### Save file
  df_fraw_allnat %>%
    write_parquet(outputsPath %>% file.path("annual_impacts_thru2050.parquet"))
} 
