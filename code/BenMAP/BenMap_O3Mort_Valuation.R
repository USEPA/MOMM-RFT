...
Inputs_mort <- file.path('Results','Global BenMAP WebTool')
Inputs_socio <- file.path('Code','InputData','Tool Inputs','RFF') #need mean population that was used in BenMAP, as well as the corresponding GDP scenario
cntryxwalkpath <- file.path('Code/InputData/Tool Inputs')
...
### country main list ###
Ctry.Name<-read.csv(file.path(cntryxwalkpath,'Final Country Grid Col_Row Index_EEM.csv'))
...
#region_vsl <- rffsp_mean_gdp_per_cap %>%
#  group_by_at(.vars = )
#  mutate(vsl = base_vsl*((gdp_per_cap/usa_base_income$gdp_per_cap)^Elasticity))
...
MortResults.Ctry <-  left_join(ozone_mort,Ctry.Name[,c('COL','Region','COUNTRY','SuperRegion',"RFF_iso_code")],
                            by=c("column"="COL")) %>%
                      ungroup #%>%
                      #select(-c('row','point_estimate','pct_2_5','pct_97_5','PointEstimate'))#
...
FinalResults.Ctry <- CombinedResults.Ctry %>%
                        ...
                        #mutate(damages      = PE_lag * vsl_calc,
                        #       damages_2_5  = PE_2.5_lag * vsl_calc,
                        #       damages_97_5 = PE_97.5_lag * vsl_calc) %>%
...
### ADD DISCOUNTING HERE#### (add percentiles - from BenMAP results) - plot by GCM in the supplement. 
...
#        # damages.baseline      = gdp * damages.baseline.pct,
         ...
         damages.marginal.97_5  = damages.delta.97_5 / 275e6
          ) %>% #to covert to $ per mt CH4 #(damages.perturbed - damages.baseline) * (12/44) * (1e-9)) %>% 
  ungroup()
