### Title: 2_Global Ozone Reduce Form Tool_forGIVE.R                                  ##
### Purpose: Adapt the MOMMM-Reduced Form Tool to accept and output country-level 
###           mortality counts for use in the GIVE model                              ####
...
endYear             = 2300
Years               <- c(BaseYear:endYear) # set simulation years
...
# User-Defined Methane Pulse Data (this will be new input absolute CH4 emissions amount)
Usr_methane_mmt  = rep(275,(2300-2020+1))
Usr_methane_ppbv = Usr_methane_mmt*methane_mmt_to_ppbv
...
RFF_TrajNumber = '1'   #Options: numerical value 1-10000, or 'All', or 'mean'
...
#new default is zero
#cessation_flag = 0 #if set to 1, with calculate results with and without lag
...
# index = RFF number, row value = mortality trajectory number (there are only 1000 unique mortality trajectories)

# 2) Set the trajectory numbers for the user-defined scenario (change to run with single trajectories number)
...
Usr_MethaneProj$Methane       <- Usr_methane_ppbv
...
response_nox_sensitivity <- read.csv(file.path(Inputs, 'CCAC',"Country_ozone_response_nox_sensitivity.csv"))#,
                                   #  col_types = cols())
